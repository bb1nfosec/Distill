...
def apply_fixes(patterns: list[WastePattern], project_path: Path) -> int:
    ...
    for ignore_path in ignore_files:
        ...
        if ignore_path.exists():
            ...
            header = f"# {ignore_path.name} — auto-generated by skim analyze --fix\n"

        new_entries = [e for e in entries_to_add if e not in existing]
        if not new_entries:
            continue

        block = "\n# Added by skim analyze --fix\n" + "\n".join(new_entries) + "\n"
...
def print_analysis(patterns: list[WastePattern], path: str):
    ...
    if not patterns:
        ...
        if auto_fixable:
            ...
            print(f"  Run: skim analyze --fix  (or pass --fix now to apply)\n")
    ...
    if args.session:
        ...
        if args.json:
            ...
            if results['largest_messages']:
                ...
                for m in results['largest_messages']:
                    print(f"    [{m['role']}] {m['tokens']:,} tokens — {m['preview'][:60]}")
